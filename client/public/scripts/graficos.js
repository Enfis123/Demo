document.addEventListener("DOMContentLoaded", function () {
  ...
  // Realizar la solicitud fetch para obtener la lista de barcos
  fetch("/api/barcos")
    .then((response) => response.json())
    .then((barcos) => {
      h2Element.textContent = "Análisis para el barco " + barcos[0].nombre;
      barcos.forEach((barco) => {
        var option = document.createElement("option");
        option.value = barco.id;
        option.text = barco.nombre;
        barcoSelect.appendChild(option);
      });
      // Establecer el valor seleccionado inicial (puedes ajustarlo según tus necesidades)
      barcoSelect.value = barcoId || barcos[0].id;

      // Luego de cargar los barcos, realizar la solicitud fetch para obtener las variables asociadas al barco seleccionado
      fetch(`/api/barcos/${barcoSelect.value}/variables`)
        .then((response) => response.json())
        .then((variables) => {
          // Limpiar las opciones actuales en el select de variables
          variableSelect.innerHTML = "";

          // Iterar sobre las variables y agregar opciones al select
          variables.forEach((variable) => {
            var option = document.createElement("option");
            option.value = variable.idVariable;
            option.text = variable.nombre;
            variableSelect.appendChild(option);
          });

          // Establecer la variable actual a partir del parámetro de la URL
          currentVariable = idVariableFromURL || variables[0].idVariable;
          variableSelect.value = currentVariable;
          // Crear el gráfico inicial con la variable seleccionada
          createInitialChart(currentVariable);

          // Manejar eventos para cambios en el select de barcos y variables
          barcoSelect.addEventListener("change", function () {
            // Al cambiar el barco, cargar las variables asociadas a ese barco
            fetch(`/api/barcos/${this.value}/variables`)
              .then((response) => response.json())
              .then((variables) => {
                h2Element.textContent =
                  "Análisis para el barco " + variables[0].nombreBarco;
                variableSelect.innerHTML = "";
                // Iterar sobre las variables y agregar opciones al select
                variables.forEach((variable) => {
                  var option = document.createElement("option");
                ...
                // Al cambiar de barco, seleccionar la primera variable por defecto
                variableSelect.value = variables[0].idVariable;
                ...
                if (currentVariable !== null) {
                  ...
                  socket.emit(
                    "unsubscribeFromDatosTemporales",
                    currentVariable
                  );
                }
                ...
                fetch("/api/variables/" + newVariable)
                  ...
                  .catch((error) =>
                    console.error(
                      "Error al obtener detalles de la variable:",
                      error
                    )
                  );
          ...
          // Suscribirse a eventos de nuevo registro para actualizar el gráfico en tiempo real
  ...
  document
    .getElementById("buscar-datos-btn")
  ...
  document
    .getElementById("guardar-datos-excel-btn")
    .addEventListener("click", async function () {
      ...
      try {
        ...
        const csvContent =
          "ID,Barco,Variable,Escala,Timestamp,Valor\n" +
          datosFijos
            .map(
              (row) =>
        ...
        if (link.download !== undefined) {
          ...
          alert(
            "Tu navegador no es compatible con esta funcionalidad. Por favor, intenta con otro navegador."
          );
        }
      } catch (error) {
        console.error("Error al obtener datos:", error);
  ...
  function createInitialChart(initialVariableId) {
    fetch("/api/variables/" + initialVariableId)
      ...
      .catch((error) =>
        console.error("Error al obtener detalles de la variable:", error)
      );
  }
      ...
      datasets: [
        {
          ...
          backgroundColor: getBackgroundColor(
            variableDetails.graficoEstadistico
          ),
  ...
  function addZoomSliders(selectedVariableId) {
    ...
    xSlider.addEventListener("input", function () {
      charts[selectedVariableId].options.scales.x.min = parseInt(
        xSlider.value.split(",")[0]
      );
      charts[selectedVariableId].options.scales.x.max = parseInt(
        xSlider.value.split(",")[1]
      );
      charts[selectedVariableId].update();
    });

    ySlider.addEventListener("input", function () {
      charts[selectedVariableId].options.scales.y.min = parseInt(
        ySlider.value.split(",")[0]
      );
      charts[selectedVariableId].options.scales.y.max = parseInt(
        ySlider.value.split(",")[1]
      );
      charts[selectedVariableId].update();
    });
  }
  ...
  function updateChartWithData(nuevoRegistro, selectedVariableId) {
    ...
    charts[selectedVariableId].data.datasets[0].data = newData.slice(
      -maxDataPoints
    );
    charts[selectedVariableId].data.labels = [
      ...oldLabels.slice(-maxDataPoints),
      formattedTimestamp,
    ];
    charts[selectedVariableId].update();
